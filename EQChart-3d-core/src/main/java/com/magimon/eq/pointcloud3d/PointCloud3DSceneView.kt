 ...
 */
internal class PointCloud3DSceneView @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
) : GLSurfaceView(context, attrs) {

    private val renderer = PointCloud3DRenderer()
    ...
    var onPointTap: ((PointCloud3DDatum) -> Unit)? = null
    var onSceneOverlayStateChanged: ((Point3DAxisOptions, Point3DCameraOptions, PointCloud3DPresentationOptions) -> Unit)? =
        null
    ...
    /**
     * Updates the cloud dataset and requests a redraw.
     */
    ...
    /**
     * Applies color and point-size styling to the renderer.
     */
        ...
        when (event.actionMasked) {
            ...
            MotionEvent.ACTION_UP -> {
                ...
                if (!moved && !scaleGestureDetector.isInProgress) {
                    renderer.pickPoint(event.x, event.y)?.let { datum ->
                        onPointTap?.invoke(datum)
                    }
                    requestRender()
                }
            }
